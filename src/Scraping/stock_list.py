...
from ..setting import send_info, error_handle
from ..Database import sql_client

@error_handle
def scraping_stock_list(url):

    send_info(f"scraping {url}")
    response = req.get(url)
    send_info(f"status_code: {response.status_code}, Now processing data...")
    ...
    send_info(f"status_code: {response.status_code}, Data processing complete")
...
def setting_stock_list():
    ...
    result = sql_client.export_data(query)['stock_id'].tolist()
    send_info("List All Stock id Done")
    return result